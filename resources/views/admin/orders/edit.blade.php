    ...
    <div class="card-body">
        <form method="POST" action="{{ route('admin.orders.update', [$order->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="fullname">{{ trans('cruds.order.fields.fullname') }}</label>
                <input class="form-control {{ $errors->has('fullname') ? 'is-invalid' : '' }}" type="text" name="fullname" id="fullname" value="{{ old('fullname', $order->fullname) }}" required>
                @if($errors->has('fullname'))
                    <div class="invalid-feedback">
                        {{ $errors->first('fullname') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.order.fields.fullname_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('table_id') ? 'is-invalid' : '' }}" name="table_id" id="table_id" required>
                    @foreach($tables as $id => $table)
                        <option value="{{ $id }}" {{ (old('table_id') ? old('table_id') : $order->table_id) == $id ? 'selected' : '' }}>{{ $table }}</option>
                    @endforeach
                </select>
